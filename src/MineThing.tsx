...
export default function MineThing() {
  ...
  const onLiveResults = (results: Results) => {
    ...
    const specificPointIndices = [10, 337, 338, 299, 297, 333]; // Indices of points to color differently
    ...
    if (results.multiFaceLandmarks) {
      for (const landmarks of results.multiFaceLandmarks) {
        ...
        for (let i = 0; i < landmarks.length; i++) {
          ...
          const isSpecificPoint = specificPointIndices.includes(i);
  ...
  return (
    <div className='flex w-screen h-screen items-center'>
      ...
      <div className='flex-col w-1/2 h-5/6'>
        <img ref={overlayImageRef} src={imageSrc} className='h-1/2' />
        <canvas ref={overlayImageCanvasRef} className='h-1/2'></canvas>
      </div>
    </div>
  );
}
